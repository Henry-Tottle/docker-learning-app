...
function loadProject(req, res, next) {
  ...
  const project = Number.isInteger(id) ? req.progress.getProject(id) : null;
...
router.post('/:id/delete', loadProject, (req, res) => {
  req.progress.deleteProject(req.project.id);
  res.redirect('/');
});
...
router.get('/:id/free/submissions/:sid', loadProject, (req, res) => {
  const sub = req.progress.getSubmission(req.project.id, Number(req.params.sid));
