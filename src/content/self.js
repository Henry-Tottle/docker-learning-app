...
const { DOCS } = require('../engine/links');

const ROOT = path.join(__dirname, '..', '..');

const DOCKERFILE = {
  '# syntax=docker/dockerfile:1': ['Opts in to the current Dockerfile syntax. Harmless on new Docker versions and protects the multi-stage COPY --from lines from surprises on old ones.', null, [DOCS.syntax]],
  'FROM node:22.23.2-bookworm-slim AS deps': [
    'Pinned to the exact Node version installed on the machine this was built on, so the container runs the same runtime the tests ran on. -slim omits compilers and docs. The stage is named deps because that is all it produces. A stricter pin would add @sha256:... after the tag, which names one exact image build rather than a label that can be re-pointed; it was left off so the file stays readable, but it is the next step for a real deployment. The first link below shows how to look up a digest and pull by it.',
    'base-images',
    [DOCS.digest, DOCS.from],
  ],
  ...
  'RUN npm ci --omit=dev --ignore-scripts': [
    ...
    'multi-stage-builds',
    [DOCS.npmCi, DOCS.npmIgnoreScripts, DOCS.multistage],
  ],
  ...
  'ENV NODE_ENV=production': ['Express disables verbose errors and enables view caching when this is set. Baked in so every container from this image gets it by default.', 'env-vars', [DOCS.env]],
  'ENV DB_PATH=/data/progress.db': [
    ...
    'env-vars',
    [DOCS.env, DOCS.volumes],
  ],
  'COPY --from=deps /app/node_modules ./node_modules': ['Brings the installed, production-only dependencies across from the deps stage. This is the only thing that stage exists to produce.', 'multi-stage-builds', [DOCS.multistage]],
  ...
  'COPY src ./src': ['The application code, copied as an explicit folder rather than COPY . . so nothing unexpected is added when the repo grows. Listed after dependencies because it changes most.', 'layer-caching', [DOCS.cache]],
  ...
  '&& rm -rf /usr/local/lib/node_modules /usr/local/bin/npm /usr/local/bin/npx /usr/local/bin/corepack': [
    ...
    'multi-stage-builds',
    [DOCS.trivy, DOCS.layers, DOCS.distroless],
  ],
  'USER node': ['Everything above needed root. The running app does not. If a request handler is ever exploited, the attacker is "node" in a container, not root.', 'non-root-user', [DOCS.user, DOCS.nodeNonRoot]],
  'EXPOSE 3000': ['Documents the listening port. Publishing is done by ports: in compose or -p on the command line.', 'expose-vs-publish', [DOCS.expose, DOCS.publish]],
  ...
  'CMD ["node", "src/server.js"]': [
    ...
    'expose-vs-publish',
    [DOCS.cmd, DOCS.nodeSignals, DOCS.sqliteWal],
  ],
};
...
const COMPOSE = {
  ...
  'ports:': [null, null],
  '- "3000:3000"': ['Forwards port 3000 on your machine to 3000 in the container. This is what makes http://localhost:3000 work; EXPOSE alone would not.', 'expose-vs-publish', [DOCS.composePorts]],
  'volumes:': [null, null],
  '- app-data:/data': ['A named volume over /data, where DB_PATH points. Rebuild the image, recreate the container, and your progress is still there. A bind mount would also work but Docker-managed storage avoids ownership problems with the non-root user.', 'volumes-vs-bind-mounts', [DOCS.volumes, DOCS.bindMounts]],
  ...
  'app-data:': ['Declares the named volume used above. compose creates it on first up and never deletes it unless asked with -v.', 'volumes-vs-bind-mounts', [DOCS.composeVolumes]],
};

const DOCKERIGNORE = {
  node_modules: ['The image installs its own with npm ci. The host copy contains a native module compiled for macOS.', 'dockerignore', [DOCS.dockerignore]],
  'npm-debug.log': ['Crash logs from failed installs; junk.', 'dockerignore'],
  '.env': ['Configuration overrides for local runs. Never inside an image, even if today it holds nothing secret.', 'dockerignore', [DOCS.dockerignore]],
